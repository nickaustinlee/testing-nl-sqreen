...
describe('Config', () => {
    ...
    describe('parseConfig', () => {
        ...
        it('should not change anything to a good conf', { plan: 11 }, (done) => {
            ...
            expect(config.ingestion_url).to.equal(orig.ingestion_url);
    ...
    describe('readConfig', () => {
        ...
        it('should read the conf from a json file with a found place', { plan: 1 }, (done) => {

            const paths = require.main.paths;
            require.main.paths = [];
            const toWrite = { url: 'http://back2.sqreen.io' };

            Fs.mkdirSync(Path.join(process.cwd(), 'xx'));
            Fs.writeFileSync(Path.join(process.cwd(), 'xx', 'sqreen.json'), JSON.stringify(toWrite));

            const read = Config.readConfig(Path.join(process.cwd(), 'xx', 'node_modules', 'node_modules', 'node_modules'));
        ...
        it('should not read the conf from a malformed json file', (done) => {
            ...
            try {
                expect(read).to.equal({ app_name: 'sqreen' });
            }
            catch (_) {
                expect(read).to.equal({ app_name: 'lab' });
            }
        ...
        it('should read the conf from a json file and env variables', { plan: 4 }, (done) => {

            const toWrite = { url: 'http://back2.sqreen.io', token: 'token', disable: true };

            process.env.SQREEN_URL = 'http://back2.sqreen.io';
            process.env.SQREEN_INGESTION_URL = 'http://back2.sqreen.io';
            ...
            expect(read.ingestion_url).to.equal('http://back2.sqreen.io');
            expect(read.token).to.equal(toWrite.token);
            expect(read.run_in_test).to.equal(true);

            delete process.env.SQREEN_URL;
            delete process.env.SQREEN_INGESTION_URL;

            done();
        });

        it('should read the conf from env variables', { plan: 18 }, (done) => {
            ...
            process.env.SQREEN_HEARTBEAT_DELAY = '120';
            process.env.SQREEN_IP_HEADER = 'my-header';
            process.env.SQREEN_APP_NAME = 'my-app';
            process.env.SQREEN_STRIP_SENSITIVE_KEYS = 'my-app,hello,     heelo    ';
            process.env.SQREEN_STRIP_SENSITIVE_REGEX = 'aaa,[1-9]*';
            process.env.SQREEN_USE_WORKSPACE = '1';
            process.env.SQREEN_WORKSPACE_DEPTH = '1';
            ...
            expect(read.app_name).to.equal('my-app');
            expect(read.strip_sensitive_keys).to.equal(['my-app','hello','heelo']);
            expect(read.strip_sentitive_regex).to.equal([/aaa/, /[1-9]*/]);
            expect(read.heartbeat_delay).to.equal(120);
            expect(read.use_workspace).to.equal(true);
            expect(read.workspace_depth).to.equal(1);
            ...
            delete process.env.SQREEN_APP_NAME;
            delete process.env.SQREEN_STRIP_SENSITIVE_KEYS;
            delete process.env.SQREEN_STRIP_SENSITIVE_REGEX;
            delete process.env.SQREEN_HEARTBEAT_DELAY;
            delete process.env.SQREEN_USE_WORKSPACE;
            delete process.env.SQREEN_WORKSPACE_DEPTH;
