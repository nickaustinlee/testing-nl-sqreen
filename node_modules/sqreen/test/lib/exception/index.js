...
const TYPE = require('../../../lib/enums/events').TYPE.ERROR;
const Tranform = require('../../../lib/signals/translate').TRANSFORMS[TYPE];

describe('Exc', () => {

    it('should translate an exception to new signals', { plan: 4 }, (done) => {

        const err = new Error('my error');
        err.ruleName = 'poney';
        err.rulesPack = 'dgf57d3f5sh74';

        const payload = Exception.toReport(err);

        const res = Tranform(payload);

        expect(res.type).to.equal('point');
        expect(res.signal_name).to.equal('sq.agent.exception');
        expect(typeof res.location.stack_trace[2].abs_path).to.equal('string');
        expect(typeof res.location.stack_trace[2].lineno).to.equal('number');

        done();
    });

    it('should translate an exception to new signals without rule', { plan: 4 }, (done) => {
