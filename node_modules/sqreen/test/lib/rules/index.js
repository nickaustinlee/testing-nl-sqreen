...
describe('Rules', () => {
    ...
    describe('_UpdatePayload', () => {
        ...
        it('should cover async hookpoints', { plan: 1 }, (done) => {

            const point =  new Rules._UpdatePayload({});
            point.getCallbacks.push(() => ({ callbacks: { async_post: 'xxx' } }));

            point.build();

            expect(point.params.asyncPostCbs[0].method).to.equal('xxx');
            done();
        });
    });
    ...
    describe('_collectHookPointsInstructions', () => {
        ...
        describe('enforceRuleList', () => {
            ...
            it('should enforce a rule list with old metrics', { plan: 9 }, (done) => {
            ...
            it('should enforce a local rule list', { plan: 9 }, (done) => {
                ...
                expect(MockedRules.rulespack).to.equal('local');
                done();
            });


            it('should enforce a rule list with a monitoring rule', { plan: 11 }, (done) => {
                ...
                MockedRules.enforceRuleList([rul]);

                expect(rul.exception_cap).to.exist();
                expect(rul.sampler).to.exist();
